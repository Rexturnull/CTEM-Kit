...
**關於 NIST SP 800-30 五級與本矩陣四級的差異：** NIST SP 800-30 Rev. 1 Table H-3 定義五個影響等級（Very High 至 Very Low）。本矩陣刻意省略 **Very Low** 等級。理由：在單一主機 CTEM 評估情境中，三個 CIA 維度皆評為 Low 的主機已經是最低有意義的評估目標。一台「Very Low」主機 — 入侵對所有維度僅產生微不足道的影響 — 本就不值得納入 CTEM 評估。決定評估一台主機本身就隱含至少 Low 等級的影響。此四級簡化不會失去可操作風險分類的精確度。
## 記錄結果
